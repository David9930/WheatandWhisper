...
// Date/Time of last Modification: February 08, 2026 - 10:00 AM EST
...
//                   PHASE 2 - Collapsible Sections & Gallery:
...
//                   - THUMBNAIL GALLERY: Shows main image + additional gallery images
...
//                   BUG FIX - Gallery Parsing:
//                   - Added YAML array parsing to handle gallery lists from CMS
...
// Version: 2.2 - Phase 2 (Thumbnail Gallery Added)
// ========================================
    ...
    if (frontMatterMatch) {
        ...
        while (i < lines.length) {
            ...
            if (match) {
                ...
                if (value === '' || value === '[]') {
                    // Look for list items on next lines
                    const listItems = [];
                    i++;
                    while (i < lines.length && lines[i].match(/^\s{2,}-\s/)) {
                        const itemMatch = lines[i].match(/^\s{2,}-\s+(.+)$/);
                        if (itemMatch) {
                            const itemValue = itemMatch[1].trim();
                            // Check if it's a nested object
                            if (itemValue.match(/^\w+:/)) {
                                const obj = {};
                                ...
                                listItems.push(itemValue.replace(/^["']|["']$/g, ''));
                            }
                        }
                        i++;
                    }
                    product[key] = listItems;
                    continue;
                }
                ...
                else if (key === 'stock' || key === 'order') value = parseInt(value) || 0;
...
function loadGalleryThumbnails(product) {
    ...
    mainThumb.src = product.image || 'images/uploads/placeholder-product.jpg';
    mainThumb.alt = product.title;
    mainThumb.className = 'gallery-thumbnail active';
    ...
    product.gallery.forEach((item, index) => {
        ...
        thumb.src = item.image || item; // Handle both {image: "..."} and "..." formats
        thumb.alt = `${product.title} - Image ${index + 2}`;
...
function setActiveThumbnail(activeThumbnail) {
    const allThumbs = document.querySelectorAll('.gallery-thumbnail');
